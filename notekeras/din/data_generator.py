        ...
        while True:
            ...
            for i in idxs:
                ...
                if len(hist_item) == self.batch_size:
                    ...
                    yield [np.array(hist_item), np.array(hist_cat), np.array(target_item), np.array(target_cate),
                           np.array(hist_len), np.array(b_label)], None
    ...
    def read_file(self):
        ...
        with open(self.file, "r") as f:
            for line in f:
                ...
                hist_len = len(hist)
                res.append(
                    [np.array([hist]), np.array([cate]), np.array([click_next_item]), np.array([click_next_item_cate]),
                     np.array([hist_len]), float(label)])
                num += 1
                if num > 10000:
                    break
        return res
