...
from notekeras.temp.RPN.rpn import RPNPlus
from notekeras.temp.RPN.utils import compute_iou, load_gt_boxes, wandhG, compute_regression
...
def create_image_label_path_generator(synthetic_dataset_path):
    ...
    image_label_paths = [(os.path.join(synthetic_dataset_path, "image/%d.jpg" % (idx + 1)),
                          os.path.join(synthetic_dataset_path, "imageAno/%d.txt" % (idx + 1))) for idx in
                         range(image_num)]
    while True:
...
def compute_loss(target_scores, target_bboxes, target_masks, pred_scores, pred_bboxes):
    ...
    score_loss = tf.reduce_sum(score_loss * foreground_background_mask, axis=[1, 2, 3]) / np.sum(
