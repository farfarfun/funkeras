...
def main(tiny=False, model='yolov4', weights=data_root + '/data/yolov4.weights'):
    trainset = Dataset(is_training=True, tiny=tiny, model=model)
    testset = Dataset(is_training=False, tiny=tiny, model=model)
    logdir = "./data/log"
    isfreeze = False
    ...
    freeze_layers = utils.load_freeze_layer(model, tiny)
    ...
    def tes_step(image_data, target):
        with tf.GradientTape() as tape:
            pred_result = model(image_data, training=True)
            giou_loss = conf_loss = prob_loss = 0
    ...
    for epoch in range(first_stage_epochs + second_stage_epochs):
        ...
        for image_data, target in testset:
            tes_step(image_data, target)
        model.save_weights(data_root + "/checkpoints/yolov4")
